...
const generateBidReport = async (req, res) => {
  ...
  try {
    ...
    if (format === 'pdf') {
      ...
      bidData.forEach((bid, idx) => {
        doc.fontSize(12).text(`Bid #${bid.bidNumber}`, { underline: true });
        doc.text(`Product: ${bid.productName}`);
        doc.text(`Brand: ${bid.brandName}`);
        doc.text(`Start Price: $${bid.startPrice}`);
        doc.text(`Highest Bid: $${bid.highestBid}`);
        doc.text(`Status: ${bid.status}`);
        doc.text(`Close Date: ${new Date(bid.closeDate).toLocaleString()}`);
        doc.text(`Total Bidders: ${bid.biddersCount}`);
        doc.moveDown(2);
      });
